use miden::{Assembler, DefaultHost, ProvingOptions, StackInputs, ExecutionProof, ProgramInfo, StackOutputs, utils::{Serializable, Deserializable}, Program};
...
impl Fibonacci{
    fn prove(&self) -> (StackOutputs, ExecutionProof, miden::Program){
        ...
        let program_info: ProgramInfo = ProgramInfo::new(program.hash(), program.kernel().clone());
        
        let program_info_serialized = program_info.to_bytes();
        println!("Program Info: {:?}", program_info_serialized);
        ...
        .unwrap();
        println!("Proof serialized: {:?}", proof.to_bytes());
        println!("Outputs serialized: {:?}", outputs.stack());
